...
const TYPE: Record<string, string> = { customer: '매출처', both: '매출/매입' }

// GET /api/customers/export — 매출처(거래처 마스터) 목록
export async function GET() {
  ...
  const result = await fetchAllRows<Record<string, unknown>>((f, t) =>
    admin.from('vendors')
      ...
      .in('type', ['customer', 'both']).eq('is_active', true).order('name').range(f, t),
  )
  if ('error' in result) return NextResponse.json({ error: result.error }, { status: 500 })
  const rows = result.data.map(v => ({
    '매출처명': v.name, '사업자번호': v.biz_number ?? '', '구분': TYPE[v.type as string] ?? v.type,
    '담당자': v.contact_name ?? '', '연락처': v.contact_phone ?? '', '이메일': v.email ?? '',
    '거래처통보잔액': v.ledger_balance ?? '', '메모': v.note ?? '',
  }))
  return xlsxResponse(rows, '매출처목록', [24, 14, 10, 12, 16, 22, 16, 30])
}
