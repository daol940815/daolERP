...
- **확정된 방향(B안 — 직원 워크스페이스 통합)**: 일반 직원에게는 개인화면(마이페이지)을
  홈으로 하는 **단일 워크스페이스**만 보여준다. 사이드바 메뉴: 내 대시보드 / 주문
  (현황, 이후 입력·발주·배송이 단계적으로 열림) / 근태·휴가(기존 /hr 화면 연결) /
  내 영업일지. 기존 /orders·/hr 화면은 재작성하지 않고 사이드바로 잇는다(배선 작업).
- **모드 선택(/portal)은 admin 전용**: 일반 직원·중간 관리자는 로그인 후 워크스페이스로
  직행(리다이렉트). admin만 워크스페이스/회계·경영 선택 화면을 본다. 중간 관리자는
  워크스페이스 사이드바에 승인 메뉴가 추가되는 형태.
- 회계 등 중요 데이터 접근 차단은 이미 구조화되어 있음 — (dashboard) 레이아웃이
  admin 외 역할을 `/orders`로 리다이렉트. middleware 가드 구조 유지.
...
- **로그인 유지 정책(2026-09-18 확정)**: 기본은 브라우저 세션 쿠키(닫으면 로그아웃, 다음
  접속에 로그인 화면). 로그인 화면의 "로그인 상태 유지" 체크 시에만 장기 쿠키
  (`daol-keep-signed-in` 기준). 아이디만 localStorage에 기억하고 **비밀번호는 절대 저장하지
  않는다**(브라우저 저장 기능에 맡김). 규칙은 `lib/auth-session-prefs.ts` 한 곳 — 브라우저
...
- **거래처 담당자 존칭(2026-08-14 확정)**: 고객사 인물은 화면·엑셀·저장 표기 모두
  ...
  자유 입력 문자열은 `honorify()`로 보충. 자사 직원(employees)은 대상 아님.
  **인물 마스터(contacts.name)에는 존칭·직함을 저장하지 않는다** — 동일인 판정·검색이
  순수 이름 기준이라 저장은 이름만, 직함은 `contact_assignments.title`, 존칭은 표시에서.
