...
| 거래 유형 | 차변 | 대변 | 금액 | 비고 |
|---|---|---|---|---|
| 은행 **입금** | 은행계좌 GL | 분류계정(confirmed) | amount_in | line.vendor_id = tx.vendor_id |
| 은행 **출금** | 분류계정(confirmed) | 은행계좌 GL | amount_out | line.vendor_id = tx.vendor_id |
...
| 매출 세금계산서 | 매출채권(1101) | 매출(확정) + 부가세예수금(2003) | 합계 | 매출채권 line.vendor_id = vendor / 면세는 세액0 |
| 매입 세금계산서 | 비용·자산(확정) + 부가세대급금(1102) | 미지급금(2001) | 합계 | 미지급금 line.vendor_id = vendor / 면세는 세액0 |

### 회계상 예외 규칙 (v1 반드시 적용)
- **은행계좌 GL 매핑**: `bank_accounts.gl_account_id`로 결정한다. 일반계좌는 **보통예금(1001)**,
  마이너스통장(`account_type='overdraft'`)은 **단기차입금(2002, 부채)**. 입금=차입 상환(차변),
  출금=차입 증가(대변)로 자동 성립한다(매핑만 바꾸면 복식부기가 맞음).
