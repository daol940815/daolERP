...
(주의: 엑셀 집계표의 228/120·102/217은 **필터 걸린 화면의 SUBTOTAL 값**으로 확인됨 —
전체 모집단 기준 실제 값은 2025 신규 1,097/이탈 1,019, 2026 신규 497/이탈 1,097.
검증 보고서 2026-07-24 참조)
...
- 종합: A=4~D=1 평균 반올림. 친밀도 미입력 시 매출·연속성 2개 평균.
  (참고: 엑셀 원본 수식은 미입력을 D(1점)로 취급해 종합 D가 과대(2,119명)했다 —
  확정 규칙은 미입력을 벌점으로 삼지 않는 쪽으로 의도적으로 변경)
...
- ⑥이 이 설계의 **검증 기준** — 2026-07-24 로컬 사전검증 완료
  (`reports/crm-validation-2026-07-24.md`): DB 등급 계산 3,301명 전수 대조 불일치 0,
  매칭 100%, 시즌 백필 98.5%(오분류 0). 엑셀과의 차이는 전부 원인 규명
  (엑셀 캐시 스테일 38건, 필터 SUBTOTAL 집계표, 통계 시트 누락 고객 290명 회수).
  운영 적용 전 테스트 Supabase에서 실제 erp_orders 대상 재검증 1회 남음.
...
| 산출물 | 위치 | 상태 |
|---|---|---|
| 테이블·인덱스·RLS·merge_vendor 확장 | `supabase/migrations/069_crm_core.sql` | 작성 완료, 로컬 검증 통과 |
| 매칭·등급·신규이탈·스냅샷·병합 RPC | `supabase/migrations/070_crm_rpcs.sql` | 작성 완료, 로컬 검증 통과 |
| 엑셀 이행 SQL 생성기 | `scripts/crm-import/build_seed.py` | 작성 완료 |
| 검증 도구 (시뮬레이션 적재 + 3단 대조) | `scripts/crm-import/validation/` | 작성 완료 |
| 사전검증 보고서 | `reports/crm-validation-2026-07-24.md` | 완료 — DB 전수 대조 불일치 0 |
| 테스트 Supabase 실데이터 재검증 | — | **대기** (사장님: 테스트 프로젝트 준비) |
| 화면 4종 + API 6종 | `app/(dashboard)/crm/*`, `app/api/crm/*` | 구현 완료, next build 통과 (실DB 연동 확인은 테스트 Supabase에서) |
