...
- **도메인 사실 확인(2026-08-10, 사용자)**: ERP 매입가(erp_order_items.purchase_total)는
  **부가세 포함 금액**이다 (면세 상품은 세액 0이라 총액=공급가일 뿐).
  ...
  → **매입 사이클(060) 재검토 미결**: 상태 엔진의 ERP↔계산서 비교가 공급가 기준
  (docs/purchase-cycle-design.md v3)인데, ERP가 부가세 포함이면 과세 매입처는 약 9.1%
  상대 차이가 상시 발생하고 TOL=10%가 이를 흡수하는 구조 — 실질 허용오차가 0.9%로
  좁아져 있음. 비교 기준을 총액으로 바꾸고 TOL을 재조정할지 매입 사이클 트랙에서 결정 필요.
