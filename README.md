...
| M2 | 근무정책·연차정책·휴가유형·휴일 관리, 정책 배정, 버전 관리 | 예정 |
| M3 | 근무일정 (자동 생성/조정/재생성) + 승인 모듈 골격 | 예정 |
| M4 | 출퇴근 이벤트, 보정 신청·승인, 근태 계산 엔진, 첨부파일 | 예정 |
| M5 | 휴가 (연차 발생/소멸, 신청·승인, 잔여 관리, 촉진) | 예정 |
| M6 | 초과근무, 52시간 모니터링, 알림 아웃박스, 퇴사 프로세스 | 예정 |
| M7 | 월 마감 (검증+스냅샷), 통계/리포트, 관리자 대시보드, Excel Import/Export | 예정 |
